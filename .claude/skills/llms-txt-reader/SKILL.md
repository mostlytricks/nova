...
- `http://localhost:3000/llms.txt` — this project's local server, master view
- `http://localhost:3000/<namespace>/llms.txt` — one namespace (e.g. `/fastify/llms.txt`)
- `http://localhost:3000/llms.txt?merge=true` — master + external sources, all in one
- `https://<some-site>/llms.txt` — a third-party doc index

If the user just says "use my llms.txt" with no URL, assume `http://localhost:3000/llms.txt` (this project's server). Confirm if unsure.
...
**You:** Fetch `http://localhost:3000/llms.txt` → see `Namespaces` section → find `/fastify/llms.txt` link → fetch that → find `Routing` entry → fetch it → answer, citing the routing doc.
...
**You:** Fetch `http://localhost:3000/llms.txt`, parse, report title + sections + entry count. Don't fetch any entries until the user asks a substantive question.
