...
description: Fetch internal or protected web resources from Windows without built-in WebFetch/browser tools, using curl.exe with custom CA bundles, explicit headers, cookies, redirects, and large-response handling. Use when Codex must retrieve URLs that need corporate/private CA trust, Authorization headers, session cookies, custom request headers, or precise Windows command behavior.
---
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .codex\skills\windows-ca-web-fetch\scripts\fetch.ps1 `
