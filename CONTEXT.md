...
## Completed
- **Rename executed in-repo: `local-llmstxt-server` → `nova`** (2026-07-22). package.json + all doc identities + changelog links renamed and pushed; GitHub repo was already `mostlytricks/nova` (old URLs redirect). `.codex/skills/local-llmstxt-server/` slug kept (Codex/astra discovery name). **Workspace folder/junction rename still pending** — the repos/ folder was locked by an open handle (editor/Explorer); retry with everything closed.
